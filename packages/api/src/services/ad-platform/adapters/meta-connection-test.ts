...
export const metaConnectionTestAdapter: PlatformConnectionTestAdapter = {
  provider: 'meta',
  ...
  async test(input) {
    if (input.provider !== 'meta') throw new PlatformConnectionTestError('AD_PLATFORM_CONNECTION_TEST_INPUT_INVALID')
    ...
    if (!PIXEL_ID_PATTERN.test(pixelId || '')) throw new PlatformConnectionTestError('AD_PLATFORM_CONNECTION_TEST_DESTINATION_REJECTED')
    ...
    const eventIds = await connectionTestEventIds('mav', input.testId)
    const eventTime = Math.floor(Date.now() / 1_000)
    const eventSourceUrl = requireProductionSiteUrl(input.siteUrl, '/attribution-test/meta')
    const response = await fetchConnectionTest(
      input.fetcher ?? fetch,
        ...
        body: JSON.stringify({
          data: (['Contact', 'CompleteRegistration'] as const).map(event => ({
            ...
            user_data: {
              ...
              client_user_agent: 'MeiGallery Attribution Test/1.0',
            },
