...
describe('归因连接快照', () => {
  ...
  it.each([
    ...
    ['额外 public config', (rows: SnapshotRow[]) => rows.map(row => ({ ...row, public_config_json: '{"pixelId":"pixel_1","unexpected":"x"}' }))],
...
function validRows(): SnapshotRow[] {
  return ['Contact', 'CompleteRegistration'].map((canonical_event, index) => ({
    ...
    public_config_json: '{"pixelId":"pixel_1"}', rollout_target_percentage: 100, rollout_effective_percentage: 100,
