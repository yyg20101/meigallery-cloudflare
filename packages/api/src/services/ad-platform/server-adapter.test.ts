...
describe('服务端 Adapter 注册表', () => {
  it('按 provider 查表选择 Adapter，未知 provider fail closed', async () => {
    ...
    await expect(deliverServerEvent({
      input: { provider: 'unknown' as never, canonicalEvent: 'Contact', externalEventId: 'mg3_unknown', eventTime: 1_784_256_123, pageUrl: 'https://meigallery.example/', destination: 'x', matchSignals: {} },
