...
import {
  ...
  type PlatformConnectionTestInput,
...
describe('平台连接测试 Adapter', () => {
  ...
  it('Meta 重试复用确定性事件编号，且不返回凭证和测试码', async () => {
    ...
    const input: PlatformConnectionTestInput = {
      testId: 'test:meta:1',
      provider: 'meta',
    ...
    const adapter = getPlatformConnectionTestAdapter('meta')!
    const first = await adapter.test(input)
    const second = await adapter.test(input)
  ...
  it('TikTok 发送同批 Contact 与 CompleteRegistration 测试事件', async () => {
    ...
    const result = await getPlatformConnectionTestAdapter('tiktok')!.test({
      testId: 'test:tiktok:1',
      provider: 'tiktok',
  ...
  it('Google 只执行 validateOnly 并校验两个 conversion action', async () => {
    ...
    const result = await getPlatformConnectionTestAdapter('google')!.test({
      testId: 'test:google:1',
      provider: 'google',
