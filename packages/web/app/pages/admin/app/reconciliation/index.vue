...
interface ReconciliationCaseNote {
  userVisible: string
  internal: string
  resolution: string
...
const notes = reactive<Record<string, ReconciliationCaseNote>>({})
...
const caseRows = computed(() => cases.value.map(item => ({
  ...item,
  note: ensureCaseNote(item.caseId),
  recoveryPreview: recoveryPreviews.value[item.caseId] ?? null,
})))
...
async function refreshCases() {
  ...
  for (const item of cases.value) {
    ensureCaseNote(item.caseId)
  }
}

function ensureCaseNote(caseId: string): ReconciliationCaseNote {
  const current = notes[caseId]
  if (current) return current
  const created: ReconciliationCaseNote = {
...
async function createForwardFix(item: WalletReconciliationCase) {
  const note = ensureCaseNote(item.caseId)
  if (note.internal.trim().length < 2 || note.userVisible.trim().length < 2) {
...
async function verifyResolution(item: WalletReconciliationCase) {
  const resolutionNote = ensureCaseNote(item.caseId).resolution.trim()
...
async function recoverWallet(item: WalletReconciliationCase) {
  ...
  if (note.recoveryResolution.trim().length < 2 || note.recoveryEvidence.trim().length < 3) {
  ...
  const idempotencyKey = recoveryIdempotencyKeys[item.caseId] ?? `wallet.recovery:${crypto.randomUUID()}`
  recoveryIdempotencyKeys[item.caseId] = idempotencyKey
  ...
  <div class="min-w-0 space-y-6">
    ...
    <section class="grid min-w-0 gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <section class="min-w-0 overflow-hidden rounded-2xl border border-[#eaded8] bg-white shadow-sm">
        ...
        <div v-else-if="caseRows.length" class="divide-y divide-[#f2e8e3]">
          <article v-for="item in caseRows" :key="item.caseId" class="min-w-0 p-5">
            ...
            <div v-if="item.status === 'claimed' && item.forwardFix.eligible" class="mt-4 grid gap-3 lg:grid-cols-2"><label class="text-sm text-stone-700">用户可见说明<textarea v-model.trim="item.note.userVisible" rows="2" maxlength="160" class="mt-1 w-full min-w-0 resize-y rounded-xl border border-[#dccbc3] px-3 py-2" /></label><label class="text-sm text-stone-700">内部处置依据<textarea v-model.trim="item.note.internal" rows="2" maxlength="320" class="mt-1 w-full min-w-0 resize-y rounded-xl border border-[#dccbc3] px-3 py-2" placeholder="说明差异原因、证据和 Runbook 步骤" /></label><div class="lg:col-span-2"><button type="button" class="min-h-10 rounded-xl bg-[#2f2622] px-4 text-sm font-medium text-white disabled:opacity-50" :disabled="operatingId === item.caseId" @click="createForwardFix(item)">创建追加式纠正申请</button></div></div>
            <div v-else-if="item.status === 'forward_fix_requested'" class="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]"><div class="min-w-0"><NuxtLink :to="`/admin/app/coin-adjustments/${item.forwardFixAdjustmentId}/review`" class="break-all text-sm font-medium text-[#c52e61] underline underline-offset-2">查看纠正申请 {{ item.forwardFixAdjustmentId }}</NuxtLink><textarea v-model.trim="item.note.resolution" rows="2" maxlength="500" class="mt-3 w-full min-w-0 resize-y rounded-xl border border-[#dccbc3] px-3 py-2 text-sm" :disabled="!isOwner" placeholder="调币独立复核入账后，填写重建验证说明" /></div><button v-if="isOwner" type="button" class="min-h-10 self-end rounded-xl border border-[#dccbc3] bg-white px-4 text-sm font-medium text-stone-700 disabled:opacity-50" :disabled="operatingId === item.caseId" @click="verifyResolution(item)">重建并验证</button></div>
            ...
            <section v-if="isRecoveryAnchor(item)" class="mt-4 rounded-xl border border-red-200 bg-red-50 p-4">
              ...
              <template v-if="item.recoveryPreview">
                <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div class="rounded-lg bg-white p-3"><p class="text-xs text-stone-500">当前余额 / sequence</p><p class="mt-1 font-semibold text-[#2f2622]">{{ item.recoveryPreview.walletBalance }} / {{ item.recoveryPreview.walletSequence }}</p></div><div class="rounded-lg bg-white p-3"><p class="text-xs text-stone-500">重建余额 / sequence</p><p class="mt-1 font-semibold text-[#2f2622]">{{ item.recoveryPreview.rebuiltBalance }} / {{ item.recoveryPreview.rebuiltSequence }}</p></div><div class="rounded-lg bg-white p-3"><p class="text-xs text-stone-500">覆盖案件</p><p class="mt-1 font-semibold text-[#2f2622]">{{ item.recoveryPreview.coveredCases.length }}</p></div><div class="rounded-lg bg-white p-3"><p class="text-xs text-stone-500">恢复动作</p><p class="mt-1 font-semibold text-[#2f2622]">{{ item.recoveryPreview.snapshotChangeRequired ? '重建并解冻' : '验证后解冻' }}</p></div></div>
                <ul v-if="!item.recoveryPreview.eligible" class="mt-3 list-disc space-y-1 rounded-lg border border-amber-200 bg-amber-50 px-8 py-3 text-sm text-amber-900"><li v-for="blocker in item.recoveryPreview.blockers" :key="blocker">{{ blocker }}</li></ul>
                <div v-else class="mt-4 grid gap-3 lg:grid-cols-2">
                  <label class="text-sm text-red-950">恢复结论<textarea v-model.trim="item.note.recoveryResolution" rows="2" maxlength="500" class="mt-1 w-full min-w-0 resize-y rounded-xl border border-red-200 bg-white px-3 py-2" placeholder="说明核对结果和快照重建理由" /></label>
                  <label class="text-sm text-red-950">证据引用<textarea v-model.trim="item.note.recoveryEvidence" rows="2" maxlength="300" class="mt-1 w-full min-w-0 resize-y rounded-xl border border-red-200 bg-white px-3 py-2" placeholder="事件、Runbook、工单或受控证据引用" /></label>
