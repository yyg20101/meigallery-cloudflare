...
const currentApiPluginPath = resolve(appDir, 'plugins/ad-platform.client.ts')
const isolatedRuntimePluginPath = resolve(appDir, 'plugins/attribution.client.ts')
const nuxtConfigPath = resolve(cwd(), 'nuxt.config.ts')
const wranglerConfigPath = resolve(cwd(), 'wrangler.toml')
...
describe('Web Tracking 架构边界', () => {
  it('独立归因运行时正式切换前，生产 Web 只使用当前 API 链路', () => {
    expect(existsSync(currentApiPluginPath)).toBe(true)
    expect(existsSync(isolatedRuntimePluginPath)).toBe(false)
    expect(readFileSync(nuxtConfigPath, 'utf8'))
      .not.toContain('attributionBaseUrl')
    expect(readFileSync(wranglerConfigPath, 'utf8'))
      .not.toContain('NUXT_PUBLIC_ATTRIBUTION_BASE_URL')
  })
