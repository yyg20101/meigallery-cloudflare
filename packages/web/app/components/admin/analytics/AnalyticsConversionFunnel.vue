...
<template>
  <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
    ...
    <div class="space-y-4">
      <div v-for="(step, index) in steps" :key="step.label" class="grid grid-cols-[4.5rem_minmax(0,1fr)_4.5rem] items-center gap-3">
        <div class="min-w-0">
        ...
        <p class="text-right text-sm font-semibold text-gray-700">{{ stepRate(step, index) }}</p>
      </div>
    </div>
  </section>
</template>
