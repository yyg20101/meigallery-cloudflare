...
<template>
  <div data-attribution-page class="min-w-0 space-y-5">
    <div data-attribution-header class="min-w-0 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
      <div class="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div data-attribution-controls class="grid min-w-0 grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
          <div data-attribution-range-group class="col-span-2 grid min-w-0 grid-cols-[repeat(2,minmax(0,1fr))] rounded-lg border border-gray-200 bg-white p-1 shadow-sm sm:inline-flex sm:w-auto">
            <button
              ...
              :class="[
                'min-h-9 min-w-0 max-w-full rounded-md px-2 [overflow-wrap:anywhere] text-sm font-medium transition-colors sm:px-3',
          ...
          <input
            ...
            :value="props.date || ''"
            aria-label="选择归因日期"
            class="min-h-9 w-full min-w-0 max-w-full rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-700 shadow-sm focus:border-gray-400 focus:outline-none sm:w-auto"
            type="date"
            @input="emit('update:date', ($event.target as HTMLInputElement).value)"
          >
          <button data-attribution-control data-attribution-refresh class="min-h-9 w-full min-w-0 max-w-full rounded-lg border border-gray-200 bg-white px-3 [overflow-wrap:anywhere] text-sm text-gray-700 shadow-sm hover:bg-gray-50 sm:w-auto" type="button" @click="emit('refresh')">
            刷新
          </button>
        </div>
      </div>
    ...
    <div data-attribution-tabs class="min-w-0 rounded-lg border border-gray-200 bg-white px-2 py-2 shadow-sm">
      <div data-attribution-tab-list class="flex min-w-0 flex-wrap gap-1">
        <NuxtLink
    ...
    <div v-if="usage" class="flex min-w-0 flex-wrap items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 text-xs text-gray-500 [overflow-wrap:anywhere]">
