...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-017 | P2-01：按当前 Wrangler schema 为 `packages/api/wrangler.toml` 和 `packages/web/wrangler.toml` 增加 observability 配置，并记录 compatibility date 更新流程。 |  |  |
| TASK-018 | P2-02：将 zip 批量导入文档拆为当前 JSON/校验任务实现和后续完整 R2 直传异步导入设计。 |  |  |
| TASK-019 | P2-03：明确受保护图片当前采用 Worker 代理流，更新 `docs/TECHNICAL_SPEC.md` 并调整误导性命名或注释。 |  |  |
