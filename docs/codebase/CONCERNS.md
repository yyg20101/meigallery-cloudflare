...
| 中 | 前端缺少自动化测试 | `packages/web` 未发现测试配置 | UI 改版、响应式和后台表单回归依赖人工 | 引入 Playwright 或 Vitest component 测试覆盖登录、详情、后台媒体管理、真实案例 |
...
| 前端测试空缺 | 项目优先完成 MVP 功能和 Workers 部署 | `packages/web/` | UI 回归发现晚 | 先加关键 smoke E2E，再补组件测试 |
...
| 高变更首页和详情页 | 扫描高变更文件 | 首页、详情页频繁迭代 | UI 回归概率高 | 对首页和详情页加截图/E2E smoke |
...
2. [ASK USER] 前端自动化测试优先选择 Playwright E2E，还是先做 Vitest 组件测试？
