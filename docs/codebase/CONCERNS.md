...
|------|------------|------|----------|------|
| 媒体访问控制错误导致对象泄露 | A01 Broken Access Control | `packages/api/src/routes/media.ts`、`packages/api/src/routes/admin/media.ts`、`packages/api/src/routes/admin/galleries.ts`、`packages/api/src/routes/admin/import-jobs.ts`、`packages/api/src/routes/cases.ts`、`packages/api/src/routes/admin/cases.ts`、`packages/api/src/utils/media-keys.ts`、`packages/api/src/utils/import-report-key.ts`、`packages/api/src/utils/cases.ts` | 服务端 session 和 rank 校验、R2 私有对象；图库封面、缩略图、受保护图片代理、后台设置封面和删除媒体前校验 R2 key 必须属于当前图库/媒体；导入错误报告下载前校验 R2 key 必须属于当前导入任务；真实案例公开图片通过 `/api/cases/images/:imageId` 代理，读取和删除前校验 R2 key 必须属于当前案例和图片 ID | 需要持续补 E2E/集成测试覆盖过期会员、低 rank、外部 URL 分支 |
| SSRF/内部地址访问 | A10 SSRF | `packages/api/src/utils/external-url.ts`、`packages/api/src/utils/cover-url.ts`、`packages/api/src/routes/media.ts`、`packages/api/src/routes/galleries.ts`、`packages/api/src/routes/search.ts`、`packages/api/src/routes/admin/media.ts`、`packages/web/app/utils/mediaUrlSecurity.ts`、`packages/web/app/components/admin/SafeExternalLink.vue` | `assertSafeExternalUrl` 拒绝 localhost 和非公网 IPv4；公开封面外链重定向、公开 API `coverUrl`、后台媒体 `thumbnailUrl`、后台封面预览和旧站迁移原始链接均复用安全媒体 URL 解析，拒绝 `http`、localhost 和非公网地址 | 其他直接 fetch 或 redirect 外部 URL 的代码需保持同样约束 |
| 公开站点设置 URL 被配置为危险协议 | A03 Injection / A05 Security Misconfiguration | `packages/api/src/utils/public-setting-url.ts`、`packages/api/src/utils/public-site-settings.ts`、`packages/api/src/routes/admin/settings.ts`、`packages/web/app/pages/admin/settings.vue`、`packages/web/app/utils/siteSettingsSecurity.ts`、`packages/web/app/components/HomeAdBand.vue`、`packages/web/app/utils/safeMarkdown.ts` | 广告、站点图标、OG 封面仅允许显式解析通过的站内相对路径或安全 HTTPS 公开 URL；规则页只允许显式解析通过的站内相对路径；规则 Markdown 链接仅允许显式解析通过的安全 HTTPS 公开 URL；公开设置接口会清空历史危险 URL 和异常 Pixel ID；前端读取设置和后台图标预览时再次归一化 icon、OG、广告、规则页和 Pixel ID；相关入口均拒绝空白、编码控制字符、本机地址和非公网地址；前端纯文本渲染，外链使用 `noopener noreferrer` | 后续若接入第三方广告脚本，需单独做隐私、CSP 和追踪合规评审 |
| 邮件模板可变内容注入 HTML 或危险链接 | A03 Injection | `packages/api/src/services/email-templates.ts` | 邮件模板对标题、验证码、会员名等可变文本做 HTML 转义；通知按钮和封面图 URL 只允许安全 HTTPS，危险 URL 回退或省略 | 后续若接入可编辑邮件模板，需要单独引入模板白名单和预览校验 |
| 联系方式手动链接被配置为危险协议 | A03 Injection / A05 Security Misconfiguration | `packages/api/src/utils/contact-link-url.ts`、`packages/api/src/utils/contact-qrcode.ts`、`packages/api/src/routes/contact-methods.ts`、`packages/api/src/routes/admin/contact-methods.ts`、`packages/web/app/utils/contactUrlSecurity.ts`、`packages/web/app/components/ContactMethodItem.vue`、`packages/web/app/components/admin/ContactQrCodeCell.vue` | 后台仅允许 `https`、`mailto`、`tel` 和受支持客户端协议；`https` 链接拒绝本机/非公网地址；公开读取和前端点击前二次校验，不安全链接回退为复制联系值；二维码公开代理和后台删除校验 R2 key 必须属于当前联系方式；后台二维码预览复用安全 URL 归一化，历史异常 URL 不直接加载图片 | 后续新增联系平台协议时必须同步白名单和测试 |
| Import Token 泄露 | A07 Identification and Authentication Failures | `packages/api/src/routes/imports.ts`、`packages/api/src/routes/admin/import-api-tokens.ts`、`packages/api/src/utils/import-token.ts` | token hash 存储、权限和 sourceBotKey 校验、过期检查；后台禁用 token 前确认目标存在，并写入脱敏审计日志 | secret 轮换和撤销流程需继续文档化 |
