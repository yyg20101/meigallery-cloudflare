...
| 联系方式手动链接被配置为危险协议 | A03 Injection / A05 Security Misconfiguration | `packages/api/src/utils/contact-link-url.ts`、`packages/api/src/utils/contact-qrcode.ts`、`packages/api/src/routes/contact-methods.ts`、`packages/api/src/routes/admin/contact-methods.ts`、`packages/web/app/utils/contactUrlSecurity.ts`、`packages/web/app/components/ContactMethodItem.vue` | 后台仅允许 `https`、`mailto`、`tel` 和受支持客户端协议；`https` 链接拒绝本机/私网地址；公开读取和前端点击前二次校验，不安全链接回退为复制联系值；二维码公开代理和后台删除校验 R2 key 必须属于当前联系方式 | 后续新增联系平台协议时必须同步白名单和测试 |
