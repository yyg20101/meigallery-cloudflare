...
|------|------------|------|----------|------|
| 媒体访问控制错误导致对象泄露 | A01 Broken Access Control | `packages/api/src/routes/media.ts`、`packages/api/src/routes/cases.ts`、`packages/api/src/routes/admin/cases.ts`、`packages/api/src/utils/cases.ts` | 服务端 session 和 rank 校验、R2 私有对象；真实案例公开图片通过 `/api/cases/images/:imageId` 代理，读取和删除前校验 R2 key 必须属于当前案例和图片 ID | 需要持续补 E2E/集成测试覆盖过期会员、低 rank、外部 URL 分支 |
