...
| 路由级测试 | 是 | 公开图库/搜索/媒体/案例/站点设置、后台设置/媒体/案例/联系方式/Import Token/外部导入 | `packages/api/src/routes/**/*.test.ts`；媒体、图库、搜索和后台媒体测试覆盖封面/缩略图外链重定向或下发时的 HTTPS 归一化与内部地址拦截；真实案例图片代理和后台删除覆盖 R2 key 所属关系校验；联系方式公开读取、后台写入和二维码代理覆盖危险链接、内部地址和二维码 key 所属关系 |
...
- 当前测试文件数：48 个 API `*.test.ts` 文件、11 个 Web `*.test.ts` 文件、1 个 Web Playwright smoke spec。
