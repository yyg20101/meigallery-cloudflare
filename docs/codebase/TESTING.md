...
| 路由级测试 | 是 | 公开图库/搜索/媒体/案例/站点设置、后台设置/媒体/案例/联系方式/Import Token/外部导入 | `packages/api/src/routes/**/*.test.ts`；媒体、图库、搜索和后台媒体测试覆盖封面/缩略图外链重定向或下发时的 HTTPS 归一化与内部地址拦截；图库媒体、真实案例图片和导入错误报告代理/下载覆盖 R2 key 所属关系校验；联系方式公开读取、后台写入和二维码代理覆盖危险链接、内部地址和二维码 key 所属关系 |
