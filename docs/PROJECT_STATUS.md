...
- Wallet-1 D1 定向测试已覆盖默认关闭、虚拟零钱包、负余额拒绝、独立复核、请求幂等、旧预览冲突、完整冲正、分录不可变、账号/游标隔离和必要通知目标。API 125 个测试文件/989 项、Web 60 个文件/301 项、脚本 51 项、lint、TypeScript/Nuxt 类型检查、Nuxt/Worker 构建、KMP Android Host Test/Debug APK 与 iOS Simulator Kotlin/Native 编译均通过；本机 Framework 链接仍因未接受 Xcode 许可被 `xcrun` 69 拦截，继续由 macOS CI 执行正式门禁。production/dev 的四个钱包开关保持关闭，`0077` 尚未执行远端 migration，也没有真实余额或调币记录；完整边界见 `docs/app/WALLET_1_LEDGER_INTEGRATION.md`。
...
- 2026-08-08 只读远端检查确认 dev 当前待执行 migration 恰为 `0075`～`0077`，未执行任何 D1 写入或 Worker 部署。由于钱包分录、申请事件和复核请求不可删除，共享 dev 不承载写入型功能 smoke；下一阶段先书面关闭 OQ-018、OQ-020、OQ-024，并评审一次性 D1 + 临时 Worker 的创建/销毁方案，再由用户明确批准远端变更。操作边界见 `docs/app/WALLET_1_DEV_VALIDATION_RUNBOOK.md`。
