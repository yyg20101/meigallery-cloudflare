...
- Meta CAPI v2：**本地修复仍不满足生产候选或正式部署条件**。活动 Meta 事件只有 `Contact`、`CompleteRegistration`；营销授权由 30 分钟服务端签名 HttpOnly receipt 决定，浏览器 API/session/上传统一走 Web 同源代理，body 只能降级；registration recovery 覆盖任意年龄缺失事实；migration `0043` 为 Graph 发送增加 D1 CAS lease，`0044` 将 Dataset Quality 快照绑定 approved contract digest；Pixel/CAPI 共同绑定当前 MetaConnection revision，生产 event ID 为服务端 HMAC 不透明值。后台连接验证与 dev Live Evidence 已拆分；breakdown 排除 duplicate diagnostic 行。资源门禁核对 migrations `0036..0044`、D1/R2/Queue/DLQ。Q5 Dataset Quality 继续 `contract_pending`，且缺少当前最终 commit 的真实远端 dev evidence；本轮不执行 dev/production 部署、远端 D1、Meta 网络 capture、push 或 `verify:release`，production rollout 必须保持 `0`。
