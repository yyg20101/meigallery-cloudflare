...
- 2026-07-14 Telegram 联系短链调整：`t.me` 当日进入 `serverHold` 并停止全球 DNS 解析后，系统生成的 Telegram 用户名链接改用 `https://telegram.me/<username>`。Owner 手动填写的完整链接按输入域名、路径和参数原样保存并返回，不做 `t.me` / `telegram.me` 相互替换；Web 单测仅阻止锚点真实导航，不再产生外网 DNS 噪声。API `1158` 项、Web `278` 项全部通过；无 migration、无生产配置变更。
