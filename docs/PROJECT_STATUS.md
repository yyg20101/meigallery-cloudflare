...
- 2026-07-16 通用 Meta 凭证迁移与验证修复：production 已将现有 `META_CAPI_ACCESS_TOKEN` 在 Worker 内存中迁入通用加密凭证库，未导出、打印或生成新 token；一次性迁移入口已删除。Meta 通用连接使用 Pixel `1277657707436781`，`Contact` / `CompleteRegistration` 绑定已启用。生产验证确认 Workflow 实例 ID 不接受冒号，现统一使用无冒号的稳定摘要 ID，并通过幂等 `createBatch` 调度；Web 同源代理仅在浏览器 Origin 与站点 Host 一致时恢复 Origin。Test Event Code `TEST67259` 已完成第 6 次生产验证，Meta API 接收两条服务器事件，同一 Pixel 的 `Contact` / `CompleteRegistration` 已在测试事件页确认处理完毕；连接 revision `7ae53aaf9fffc38c4d76b97e` 与凭证 revision `508a2ecf834051a99de66ab6` 一致，Server target/effective 均为 `10%`。生产 API 版本为 `31df54df-bbc6-4c77-8765-da5e1670f8fb`，对应 release `v0.4.5` / commit `320dfbe37f3dd73fb0c985ffc898c7c87d6a7cfc`。
