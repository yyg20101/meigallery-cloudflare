...
- 规划：站内一方数据分析能力已形成可落地需求方案，覆盖邀请注册、访问来源、访问链路、点击次数、点击频率、浏览时长、完整埋点字典、D1 迁移拆分、API 契约、前端 SDK、后台页面和测试矩阵；当前代码仍只有图库浏览量、点赞计数、后台基础概览和 Facebook Pixel 辅助埋点，后续实现以 `docs/PRD_DATA_ANALYTICS.md` 为需求入口。
...
- 数据分析需求方案见 `docs/PRD_DATA_ANALYTICS.md`，当前已细化到工程落地规格，但仍是后续实现草案，不代表已有生产能力。
