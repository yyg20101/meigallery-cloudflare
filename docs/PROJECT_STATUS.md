...
- 已进入逐域纵向切片：`contracts/app-api-v2.openapi.yaml` 的 M0 公共发现四个只读路径保持冻结，累计契约版本已以兼容新增方式提升到 `1.15.0`，包含 production 默认关闭的 Auth-1、Interaction-1/2/3、Search-1/2、Taxonomy-1、Membership-1/2、Message-1/2/3、Safety-2 独立复核与 Wallet-1 契约；Interaction-2/3、Search-1/2 与 Taxonomy-1 当前只完成服务端开发代码，客户端、配置与验证后置，媒体访问仍按域冻结。
...
- Search-2 当前未修改 Wrangler、未执行 `0082`、未切换搜索/taxonomy/会员目录、未迁移 grant、未实现 KMP/Nuxt 页面，也未运行 migration、专项测试或远端联调；所有现有环境继续返回 `search.filters=false`、`search.savedFilters=false`。完整边界见 `docs/app/SEARCH_2_FILTERS_AND_SAVED_FILTERS_INTEGRATION.md`。
