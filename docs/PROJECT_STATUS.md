...
- 2026-07-16 通用 Meta 凭证迁移与验证修复：production 已将现有 `META_CAPI_ACCESS_TOKEN` 在 Worker 内存中迁入通用加密凭证库，未导出、打印或生成新 token；一次性迁移入口已从生产版本 `804929fc-6e39-4a0d-b2eb-0c0c6a46fd1a` 删除。Meta 通用连接使用 Pixel `1277657707436781`，`Contact` / `CompleteRegistration` 绑定已启用，Server target/effective 保持 `0%`。生产验证确认 Workflow 实例 ID 不接受冒号，现统一使用无冒号的稳定摘要 ID，并通过幂等 `createBatch` 调度；Web 同源代理仅在浏览器 Origin 与站点 Host 一致时恢复 Origin。待发布后使用 Meta 当前 Test Event Code 重跑。
