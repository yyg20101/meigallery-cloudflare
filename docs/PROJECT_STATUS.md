...
- Taxonomy-1 KMP 已在 `meigallery-client` 提交 `5ea8dd8`：Recommendation 与 Search 共用通用稳定目录领域和 transport，公共目录执行 capability 校验、进程内缓存、ETag 条件重验证、父级/重定向/类型完整性检查。当前仍未修改 Wrangler、未执行 `0081`、未导入 legacy 标签、未实现 Nuxt 管理页面，也未运行专项测试、模拟器/真机或远端联调，所有现有环境继续返回 `taxonomy.catalog=false`。细粒度分类权限、敏感词升级审批、影响预览、灰度/回滚和迁移批次后置；完整边界见 `docs/app/TAXONOMY_1_CATALOG_AND_PROFILE_INTEGRATION.md`。
