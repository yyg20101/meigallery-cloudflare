...
- 已进入逐域纵向切片：`contracts/app-api-v2.openapi.yaml` 的 M0 公共发现四个只读路径保持冻结，累计契约版本已以兼容新增方式提升到 `1.17.0`，包含 production 默认关闭的 Auth-1、Interaction-1/2/3、Search-1/2、Taxonomy-1、Recommendation-1、Membership-1/2、Message-1/2/3、Safety-2、Wallet-1 与 Privacy-1 契约；Membership-3/4 与 Audit-1/2/3 只扩展管理员 API。Privacy-1、Recommendation-1、Search-1/2、Taxonomy-1 与 Interaction-2/3 已完成 Cloudflare 与 KMP 开发，全部新增配置与验证统一在开发阶段结束后执行，媒体访问继续按域冻结。
...
- Nuxt 已新增 `ADM-PRI-01/02` 数据权利队列与详情，提供治理门禁、脱敏申请、领取、开始处理、失败、重试和证据核验取消；没有真实制品/不可逆处理证据时不提供“完成”按钮。Operations-1 新增逾期申请聚合检测。KMP 已在 `meigallery-client` 提交 `25d4397`、`498030d`，接入系统安全状态凭证、手机/宽屏页面、注销影响逐项确认、退出登录后的申请级访问，以及注销成功响应丢失时对原幂等命令的窄化恢复。当前未修改 Wrangler、未执行 `0094`、未运行专项测试或跨仓联调，所有环境继续默认关闭；真实导出制品和不可逆删除属于 Privacy-2。完整边界见 `docs/app/PRIVACY_1_DATA_RIGHTS_CONTROL_PLANE_INTEGRATION.md`。
