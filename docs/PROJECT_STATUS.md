...
- 已进入逐域纵向切片：`contracts/app-api-v2.openapi.yaml` 的 M0 公共发现四个只读路径保持冻结，累计契约版本已以兼容新增方式提升到 `1.4.0`，包含默认关闭的 Auth-1 账号访问、Interaction-1 喜欢/关注和 Membership-1 五级会员契约；收藏、历史、消息、钱包和媒体访问仍按域冻结。
