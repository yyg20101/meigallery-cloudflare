...
- 2026-07-15 通用广告归因平台设计：书面评审已确认，实施计划已记录在 `docs/superpowers/plans/2026-07-15-unified-ad-attribution-platform.md`。Meta/TikTok 的连接、加密凭证、测试挑战、人工证据、rollout、incident 和质量快照将迁入通用控制面；统一 `Contact` 为安全外部联系链接激活，复制、二维码和面板展开不再作为广告转化。迁移采用有终点的 expand/contract，最终删除旧平台专属控制表、双写和兼容代码；实施期间 Meta production 保持既有 `10%`，TikTok 保持 disabled / rollout `0%`。
...
- `docs/superpowers/specs/2026-07-15-unified-ad-attribution-platform-design.md`：Meta/TikTok 通用控制面、加密凭证库、TikTok 验证放量和 Google 扩展设计。
