...
- 发布 PR #8 的 Playwright smoke 已补 mock API 站点设置重置入口，避免后台 SEO 保存测试污染后续断点的首页 SSR SEO 基线。
- 首页广告位外链持续增强：前台 CTA 已补离站可感知名称、离站/隐私提示，并将广告外链 `rel` 扩展为 `noopener noreferrer nofollow sponsored`。
