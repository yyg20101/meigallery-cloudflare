...
- 2026-07-14 v0.3.2 发布门禁修复：归因转化 API 强制平台隔离后，local-runtime smoke 同步显式使用 `provider=meta`，并通过真实营销授权与 `/api/ad-attribution` 签发 Meta 来源 receipt，再创建联系和注册事实；测试替身同时拒绝缺少平台或 receipt 的旧调用。首次 production 部署在 Worker deploy 前按预期阻断，D1 无待执行 migration，线上版本和 Meta/TikTok 运行状态未改变。
