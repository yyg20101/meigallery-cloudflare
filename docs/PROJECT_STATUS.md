...
- 2026-08-08 只读远端检查确认 dev 当前待执行 migration 恰为 `0075`～`0077`；本阶段只实现并本地验证工具，实际一次性 gate 仍为 `remoteSmokeAuthorized=false`，未创建远程 D1/Worker，未执行共享 dev migration 或任何远端写入。下一步仍是书面关闭 OQ-018、OQ-020、OQ-024，取得短期 gate 和用户当次批准后先运行隔离 smoke；通过也不自动放行共享 dev。操作边界见 `docs/app/WALLET_1_DISPOSABLE_SMOKE_RUNBOOK.md` 与 `docs/app/WALLET_1_DEV_VALIDATION_RUNBOOK.md`。
