...
- 2026-07-15 通用广告归因平台设计：已确认将 Meta/TikTok 的连接、加密凭证、测试挑战、人工证据、rollout、incident 和质量快照迁入通用控制面；统一 `Contact` 为安全外部联系链接激活，复制、二维码和面板展开不再作为广告转化。迁移采用有终点的 expand/contract，最终删除旧平台专属控制表、双写和兼容代码；TikTok 仍保持 disabled / rollout `0%`，设计书面评审通过前不开始实现。
