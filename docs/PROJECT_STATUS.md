...
- Telegram 外部导入当前边界明确为平台提供 API 接收端，不内置 Telegram Bot 本体；已支持 Import Token 管理、`file_id` 接收、幂等、异步拉取、R2 入库、草稿创建、Bot 侧重试、后台记录查看和后台失败重试，对接契约见 `docs/TELEGRAM_IMPORT_API.md`。
