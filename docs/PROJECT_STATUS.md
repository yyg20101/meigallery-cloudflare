...
- App 1.0 客户需求确认包已完成并升级为逐页高保真交付：`docs/app/MEIGALLERY_APP_1_0_CLIENT_PRD.md` 汇总产品范围、细节交互、异常状态、后台流程、数据迁移、验收标准和 17 项客户确认参数；`docs/app/interactive-prototype/index.html` 提供 8 个关键旅程；`docs/app/interactive-prototype/pages.html` 为移动端 49 页和管理后台 43 页提供共 92 个可独立访问、搜索、切换状态和操作的页面设计；`docs/app/APP_PAGE_LEVEL_PRODUCT_DESIGN.md` 提供逐页目录、跨页旅程和验收方法；`docs/app/deliverables/MeiGallery_App_1.0_逐页交互设计确认册.docx` 提供 17 页客户逐页确认附件。原型是需求确认与交互演示资产，不是生产 App 实现代码；下一步应按 Page ID 组织客户评审并关闭第 17 章参数。
...
- `docs/app/MEIGALLERY_APP_1_0_CLIENT_PRD.md`：客户需求确认基线，含详细功能交互、9 组高保真原型图、验收和 17 项待确认参数。
- `docs/app/interactive-prototype/index.html`：App 1.0 高保真关键旅程原型，覆盖 8 个移动端与后台旅程。
- `docs/app/interactive-prototype/pages.html`：App 1.0 逐页交互设计库，覆盖移动端 49 页和管理后台 43 页。
- `docs/app/APP_PAGE_LEVEL_PRODUCT_DESIGN.md`：92 页逐页产品设计目录、全局交互、跨页旅程和验收方法。
- `docs/app/deliverables/MeiGallery_App_1.0_产品需求确认书.docx`：供客户评审、填写意见和签字确认的完整交付版。
- `docs/app/deliverables/MeiGallery_App_1.0_逐页交互设计确认册.docx`：供客户按 92 个 Page ID 逐页评审、记录修改意见并签字确认的 17 页附件。
...
- `docs/app/MOBILE_APP_INTERACTION_SPEC.md`：移动端 Screen ID、页面目录、关键旅程、页面结构和页面级验收。
- `docs/app/ADMIN_CONSOLE_INTERACTION_SPEC.md`：Nuxt 后台 Page ID、角色、工作台、审批状态、并发、页面结构和后台验收。
