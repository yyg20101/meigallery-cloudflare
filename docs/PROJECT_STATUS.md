...
- 已进入逐域纵向切片：`contracts/app-api-v2.openapi.yaml` 的 M0 公共发现四个只读路径保持冻结，累计契约版本已以兼容新增方式提升到 `1.19.0`，包含 production 默认关闭的 Auth-1、Interaction-1/2/3、Search-1/2、Taxonomy-1、Recommendation-1、Membership-1/2、Message-1/2/3、Safety-2、Wallet-1、Privacy-1、Media-1，以及 App Core-1 运行策略、帮助与系统状态契约；Membership-3/4 与 Audit-1/2/3 只扩展管理员 API。当前全部新增配置与验证统一在开发阶段结束后执行。
