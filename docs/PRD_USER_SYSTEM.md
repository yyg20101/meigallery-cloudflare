...
当前实现备注：用户名注册/登录、邮箱验证开关、后台用户编辑/重置密码/活动日志、个人设置页、头像上传、通知偏好和邮箱修改 API 已实现；后续仍需按真实运营继续补齐更细粒度的前端验收和邮件服务配置验收。邮箱验证关闭时，修改邮箱不依赖 Email Service；邮箱验证开启后，验证码发送和送达率需单独验收。
...
- 用户名已存在、格式非法或命中保留词时，前端展示明确错误，后端返回 400，不修改原用户名。
- 修改密码时旧密码错误返回 400；连续错误的速率限制按登录/敏感操作策略执行。
- 头像 MIME、magic bytes 或大小校验失败时，后端返回 400，不写入 R2，不更新 `users.avatar_key`。
- R2 上传失败时，前端展示可重试状态，后端不更新用户头像记录。
- 修改邮箱时新邮箱已存在返回 409；验证开启且验证码过期或错误时不修改邮箱。
- 个人设置页所有写操作必须要求登录状态；未登录返回 401 并跳转登录。
