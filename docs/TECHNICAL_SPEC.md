...
- 当前开发验证只开放 `platform_managed`，公开披露为“消息由平台运营接收”；`self_managed` 只保留 schema 兼容位，不开放后台选择。公开列表、地区目录、人物详情、搜索、推荐、运营精选、关注更新、收藏/喜欢/浏览历史写入目标、平台话题发送目标与通知深链均复用同一资格表达式，统一强制认证、发布、用途授权及有效期、可见性、有效发布时间、来源图库发布和 `operation_mode = 'platform_managed'`；管理端精选校验与运营指标也使用同一口径，不能仅依赖管理写入流程的先验校验。
