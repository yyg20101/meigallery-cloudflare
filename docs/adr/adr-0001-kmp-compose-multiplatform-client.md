...
supersedes: "D-005: React Native + TypeScript 推荐基线"
superseded_by: ""
---
...
独立交友 App 首期需要覆盖 Android 和 iOS，后续需要提供面向普通用户的 Windows 和 macOS 客户端。客户端要承载登录、资料、发现、实时聊天、安全中心、会员和金币等一致业务，同时接入商店支付、推送、相机、定位、安全存储和桌面系统能力。
...
- **DEC-002**：Android 和 iOS 是 M1/M2 首发目标；Windows 和 macOS 是 M3 的面向用户桌面目标；Linux 只保留技术兼容性，不承诺首批商业发布。
...
- **POS-002**：桌面端直接复用聊天、匹配、安全和契约测试，而不是另建一套 JavaScript 桌面运行时。
...
- **NEG-005**：客户端无法直接导入现有 TypeScript 类型，需要建设契约生成、兼容检查和双语言发布流程。
...
- **ALT-004**：**Rejection Reason**：原生控制力最强，但同一交友产品需要维护三套页面和状态矩阵，不符合当前团队希望为桌面端复用 UI 的目标。平台高风险页面仍可按例外采用此模式。
...
- **REF-006**：[独立交友 App 技术架构](../app/TECHNICAL_ARCHITECTURE.md)
