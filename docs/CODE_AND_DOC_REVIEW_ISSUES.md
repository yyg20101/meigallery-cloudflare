...
- 历史发现：`packages/web` 未发现前端单测、组件测试或 Playwright E2E 配置。
...
- 已完成（2026-05-29）。
- `packages/api/src/utils/turnstile.ts` 已抽取统一 Turnstile 配置检查和 siteverify 校验逻辑。
- 认证路由继续覆盖登录、发送验证码和无邮箱验证码注册；邮箱验证码开启时，注册流程通过发送验证码接口完成人机验证。
- 后台没有独立登录端点，管理员进入后台复用普通登录入口。
- `packages/api/src/routes/admin/import-jobs.ts` 已为后台导入任务创建和处理补充 Turnstile 校验；校验失败不会创建任务，也不会把任务提前标记为 `processing`。
- `packages/web/app/pages/admin/import/**` 已在创建任务和处理任务表单中挂载 Turnstile 并提交 token。

**证据**

- `docs/TECHNICAL_SPEC.md` 和 `docs/DEPLOYMENT.md` 写后台登录、批量导入上传需要 Turnstile。
- 整改前实现中的 Turnstile 主要覆盖登录、注册、验证码发送。
- 后台没有独立登录端点，整改后文档已改为“复用普通登录入口”。
- 整改前批量导入入口未见 Turnstile 校验，整改后已接入后台导入任务创建/处理。
...
1. 已明确后台复用普通登录入口，文档改为“登录入口统一验证”。
2. 已对后台导入任务创建和处理增加 Turnstile 校验。
3. 后续新增高风险后台操作时，继续按安全验收清单评估是否接入 Turnstile 或更强 admin session 校验策略。
...
- 已完成（2026-05-29）。
- `docs/PRD.md` 已新增状态标签说明，并为用户故事、技术规格、容量限制、错误处理、监控、路线图等主要章节标注 `[当前实现]`、`[部分实现]`、`[后续规划]` 或 `[历史参考]`。
- `docs/TECHNICAL_SPEC.md` 已新增状态标签说明，并为认证、权限、媒体访问、API、D1 schema、导入、WordPress 迁移、缓存和测试范围等主要章节标注状态。
...
1. 已为 PRD 和技术设计主要段落增加状态标签：`[当前实现]`、`[部分实现]`、`[后续规划]`、`[历史参考]`。
2. 历史计划继续以 `docs/PROJECT_STATUS.md` 的归档说明为准，不作为当前生产状态依据。
3. 后续新增功能计划应单独放入 roadmap、issue 或带状态标签的文档段落，避免混入当前验收清单。
...
1. 已接入 TypeScript / Vue 兼容的 ESLint flat config。
2. 已用 `.editorconfig` 建立统一基础格式策略，避免首次接入时大面积重排历史文件。
3. CI 已增加 lint，并以 warning 方式渐进启用历史技术债清理。
