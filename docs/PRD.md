...
- zip 导入当前验收范围为解析、校验、任务记录和结构化错误返回；大文件异步完整导入不作为当前上线阻断项。
- 旧站导入后，100% 内容进入草稿或待审核状态，不允许未经审核直接发布。

后续压测和规划指标：

- 在 100,000 条图库记录规模内，组合标签搜索 P95 响应时间 <= 500 ms。
- 100 个图库目录的合法导入包，完整异步导入成功率 >= 98%。
...
- 当前可验收范围：支持上传 zip 导入包并创建导入任务记录。
- 当前可验收范围：系统校验目录结构、必填文件、CSV 字段、文件类型、重复 slug 和资源大小，并返回结构化错误。
- 当前可验收范围：合法图库默认进入草稿或待审核状态，不直接发布。
- 当前可验收范围：单个图库校验失败不影响其他图库的校验结果。
- 后续完整异步导入范围：媒体上传、草稿图库创建、错误报告 CSV 下载、失败项重试和部分成功统计。
...
| 签名 URL 有效期 | 图片 10 分钟；视频 4 小时（Stream 接入后） | 平衡安全和用户体验 |
...
- 管理员手动发放会员等级和有效期。
- 后台图库、标签、媒体、站点设置管理。
- zip 批量导入解析、校验、任务记录和结构化错误返回；完整异步媒体导入后续收敛。
- WordPress 旧站导入和迁移预览。
...
| 媒体防盗链被绕过 | 资源泄露 | R2 private bucket + Worker 签发短期 URL；Stream 接入后使用 signed token；前端不暴露原始地址 |
