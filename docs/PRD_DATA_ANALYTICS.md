...
- 口径边界：后台数据分析中的 `fb`、`facebook`、`meta` 等来源名表示站内 UTM、推广链接或 referrer 归因，不是 Meta Pixel 回传数据。
...
- 首期站内一方分析为主，Facebook Pixel 只保留第三方投放辅助，不作为后台报表数据源；后台出现 `fb`、`facebook`、`meta` 时统一解释为站内链接来源或 referrer 来源。
