...
| `P0.f` | 输出实施计划并等待确认 | 进行中 | 本文为待确认方案；确认前不批量改写 Figma 内容 |
...
1. 补齐颜色、间距、圆角、尺寸、透明度、描边、动效和断点变量。
2. 统一 Noto Sans SC 文字样式和数字/金额排版。
3. 建立 Tabler Icon Library、Icon 槽位和 48dp 点击容器。
4. 建立移动端和后台核心组件及状态变体。
5. 将页面级按钮组件迁移到正式组件库。
...
未确认前，只保留本轮审计证据和文档，不批量修改 Figma 正式内容。
