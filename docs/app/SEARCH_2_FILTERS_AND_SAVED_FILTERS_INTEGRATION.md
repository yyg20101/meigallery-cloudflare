...
状态：Cloudflare、KMP 与 Nuxt 管理核查开发完成，所有现有环境默认关闭；配置、migration 执行、专项测试与远端联调统一后置
...
本阶段不实现热门词、联想词、个性化推荐信号、在线支付、系统推送、实时搜索或后台可视化策略编辑。
...
`GET /api/admin/app/search/overview` 与 `/admin/app/search` 提供 Search-2 上线前只读核查工作区，并作为 `ADM-TAX-01` 发现目录治理的运维子视图，不新增客户侧可执行功能 Page ID。它覆盖：

- 人物搜索、搜索历史、结构化筛选和保存条件四条端到端门禁及全部阻断原因；
- 不可变搜索策略版本、当前配置选择、能力位、隐私保留门禁和安全上限；
- 当前 taxonomy 目录、父子/合并 closure 完整性和会员目录 Search-2 entitlement 执行值；
- 搜索历史偏好、有效/过期历史和保存条件的聚合健康计数；
- 保存条件来源目录、缺失/重定向/弃用/受限引用及目录迁移待复核数量。

管理 API 只返回版本、非敏感开关和聚合计数，不返回用户标识、搜索词、保存条件名称或具体 term 组合。页面不提供策略原地编辑、目录切换、grant 迁移或“一键启用”；这些动作仍必须在统一配置阶段按受控流程执行。
...
6. 完成已实现 KMP 筛选页、保存条件页的模拟器/真机与端到端交互回归。
...
- 管理端能在不读取用户搜索内容的前提下定位策略、目录、entitlement 和保存条件引用阻断，且不能直接改变运行配置。
