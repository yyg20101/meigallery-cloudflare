...
App 1.0 当前定义 99 个独立页面设计对象：

| 平台 | 页面数 | 设计范围 |
|------|--------|----------|
| Android/iOS 移动端 | 50 | 启动认证、发现真人、单向互动、平台话题、站内通知、五级会员、金币钱包、我的与系统状态 |
| Nuxt 管理后台 | 45 | 总览异常、真人供给、认证发布、目录推荐、代运营、安全申诉、会员金币、通知审计与数据权利 |

页面优先级为 P0 57 页、P1 32 页、P2 10 页。Figma 最终设计覆盖全部 99 个 Page ID 和 408 个需求状态，其中移动端 208 个、管理后台 200 个；`30｜Prototype Flows` 同步提供 99 个流程预览。为了支持脱离 Figma 的逐页签署，客户文档下一次生成将包含 99 张默认状态、57 张 P0 关键状态，以及通知/金币 23 张逐状态本地导出图，共 179 个确定性图片映射。

每个页面必须有稳定 Page ID、设计路由、页面目标、进入方式、主次操作、必备状态、成功出口、失败下一步、需求追踪键和页面级验收。客户可在 [逐页设计库](./interactive-prototype/pages.html) 搜索 Page ID 并单独评审；评审意见应引用 Page ID，涉及业务规则变化时还应引用对应 PRD/SCP 编号，避免只描述截图位置。99 页是完整需求覆盖，不代表 99 页同时进入首批开发；P0/P1/P2 的排期边界以第 3.1 节和产品决策基线为准。
...
| 交付区域 | 覆盖 | 客户评审重点 |
|---|---:|---|
| `10｜Mobile Pages` | 50 页 / 208 状态 | 页面层级、文字排版、Icon、44dp 热区、状态与返回路径 |
| `20｜Admin Pages` | 49 页 / 200 状态 | 工作台密度、筛选表格、审批、审计、数据权利、异常与并发状态 |
| `30｜Prototype Flows` | 99 个流程预览 / 614 个流程动作 | 从入口到成功、失败、受限和安全出口是否完整 |
| 页面内交互 | 2,019 个动作 | 主次操作、跨页目标、危险操作和幂等反馈 |
| `40｜Delivery Index` | 99 个 Page ID | 需求、页面、状态、原型和交付证据的定位 |
| `50｜QA & Handoff` | 视觉/交互/边界/交付门禁 | 是否达到实现、测试和客户签署所需完整度 |

QA 已按当前实时文件重新完成：408 个正式状态、99 个流程预览和 3,585 个正式页面/流程动作缺失目标为 0；移动端 208 个正式状态与 171 张支持稿不足 44dp 的目标为 0，文字样式、字体、字符图标和文字溢出问题均为 0。原 402 状态与 3,571 个动作只作为 `APP-SET-08` 增量前历史基线。客户审阅时可先查看下列最终视觉总览，再到附录 A/B/C 按 Page ID 逐页确认功能和状态。
...
![移动端 Prototype Flows 历史总览；APP-INT-06 以 Figma 实时文件为准](./assets/figma-qa/phase3/prototype-flows-overview-20260730.png)

![管理后台历史 Prototype Flows 总览；当前为 45 页](./assets/figma-qa/phase3/admin-prototype-flows-overview-20260730.jpeg)

![Delivery Index 历史总览；当前 99 个 Page ID 以 Figma 实时文件为准](./assets/figma-qa/phase3/delivery-index-summary-20260730.jpeg)
...
3. App 1.0 只展示服务端已启用的邮箱身份：登录使用邮箱和密码，注册增加邮箱验证码；手机号和第三方方式必须先扩展服务端契约、完成 Figma 页面状态并重新评审后才可开放。
4. 注册前逐项展示服务条款、隐私政策、平台代运营说明和必要资格说明；文档未加载时不能提交同意，登录遇到文档版本更新时也必须重新确认。
5. 注册成功只创建观看者账号，可跳过首次偏好直接进入非个性化推荐。
6. 登录成功后恢复账号范围内的会员、互动、会话和设置，不恢复已失效权限。

关键交互：

- 注册邮箱验证码发送按钮显示倒计时、失败原因和再次发送入口，避免重复请求。
- 登录失败区分输入错误、账号受限、频控、网络失败、风险验证和当前文档需要重新确认。
- 登录、注册或发送邮箱验证码需要 Turnstile 时进入 APP-AUTH-04；页面只表达人机安全验证，不出现手机号、短信或“确认本人”。等待态自动完成并只继续一次原操作，失败可重载挑战，服务端频控时禁用重试；任何 token 只在内存中单次使用。
...
页面结构严格对应 Figma `APP-DSC-01`：顶部为 44dp 地区、搜索和筛选入口，不在本页放置通知按钮；其下为推荐/地区/热门/最新频道、横向重点真人卡、双列推荐卡和四 Tab 导航。

真人卡片至少展示：审核封面、展示名、“资料已认证”说明、模糊地区、职业/核心标签和推荐理由。不得展示在线、精确距离、本人回复率、消费排名或快速匹配。

交互规则：

- 点击卡片主体进入 APP-DSC-07；当前 Figma 首屏只提供独立喜欢快捷操作，关注和收藏在详情或对应列表完成，不在卡片上发明未设计按钮。
- 推荐、热门和最新切换服务端排序；地区频道打开 APP-DSC-02。切换条件重置游标，不能拼接旧页。
- 接近列表末尾自动分页；分页失败保留已加载卡片和滚动位置，以非阻断反馈重试。
- 规则版本、推荐会话或目录版本变化时进入独立“规则刷新”状态，旧排序不能继续标记为当前结果。
- 卡片操作可以先反馈，但服务端失败时必须回滚并显示明确原因。
- 普通字号使用双列卡；小屏、大字体或屏幕阅读器模式切换单列。
- 个性化关闭后新推荐会话不使用账号行为信号，并显示“非个性化推荐”。
- 离线仅可复用 sort、推荐模式和地区 code 完全一致的本次运行缓存；显示缓存时间，并禁用喜欢和筛选提交。

正式状态：正常、首次空、骨架、分页、离线缓存、规则刷新。对应 Figma Frame 为 `159:61979`、`159:62126`、`159:62283`、`159:62441`、`159:62597`、`159:62753`；支持 Section `581:2` 覆盖频道、喜欢和失败反馈。

### 8.3 地区、分类、搜索与筛选

地区只使用国家、地区组、省/市等模糊层级，不请求持续定位，不显示精确距离。APP-DSC-02 使用 Figma 固定的 460dp 底部弹层：隐私说明、当前地区、地区范围、常用城市和应用动作自上而下排列；遮罩、关闭和每个选项均具有独立 44dp 热区。

基础搜索覆盖审核后的展示名、地区、职业和标签。筛选至少包含地区、职业、风格、性格、发型、服饰、场景和内容类型中已启用的目录项。

交互规则：

- 地区范围或城市点击只更新弹层草稿，点击“应用地区”后才关闭并重新请求；点击遮罩或关闭按钮放弃草稿。
- 选择更高层级范围时清除不兼容城市，选择城市时提交该城市稳定 code；“全国”提交 null。重复应用当前值只关闭弹层，不发起无效刷新。
- 地区列表超出首屏时横向滚动；“定位未使用、目录更新、无结果”分别使用独立 Figma 状态，不申请系统定位权限。
...
地区正式状态对应 Figma Frame `159:62911`、`159:63100`、`159:63298`、`159:63496`；支持 Section `603:2326` 覆盖同城、华南、全国、上海、苏州、成都等选择联动。客户端请求只使用服务端稳定地区 code 和目录版本，不得把“华东”“杭州”等中文展示名作为查询键。

分类页 APP-DSC-03 的正常、空分类和目录失效正式稿对应 `159:63697`、`159:63800`、`159:63865`；支持 Section `627:2770` 提供加载中、加载失败和目录变化说明。内容主题、职业与身份、风格分别进入 APP-DSC-05 对应目录类型，地区进入 APP-DSC-02，本周主题直接使用服务端 stable term ID 执行搜索；客户端不得按展示名猜测已合并或下线分类。

搜索页 APP-DSC-04 的初始、输入中、有结果、无结果和历史关闭正式稿对应 `159:63946`、`159:64061`、`159:64147`、`159:64252`、`159:64339`。初始态提供最近搜索、热门发现、全部分类、筛选、已保存条件与“管理历史”；输入态可清空、使用公开索引建议或提交；结果态可进入真人详情或平台认证规则；无结果只允许用户主动修改搜索词、放宽筛选或查看热门推荐；历史关闭态提供显式重新开启入口。支持 Section `645:2770` 另覆盖搜索中、搜索失败、分页中、分页失败、历史加载中、历史加载失败、历史为空、清空历史确认、平台认证规则、历史管理、单条删除中和单条删除失败 12 张交互稿，历史为空态可再次关闭后续记录。管理页支持逐条删除、清空全部和关闭后续记录；单条删除期间只锁定当前任务，失败时保留全部权威记录并提供定向重试。清空确认明确区分“仅清空”和“清空并关闭记录”，遮罩下控件不得点击穿透；首屏或分页失败均保留当前任务与已验证结果。
...
| `APP-WAL-03` | 金币分录详情 | 正常、业务单号已复制、分录不可用、扣减 |

这 23 张逐状态导出图是 408 个 Figma 最终状态中的深度确认样本；当前最终稿覆盖移动端 50 页/208 状态、后台 49 页/200 状态。当前 3,585 个正式页面/流程动作及全量 QA 已复核完成；3,571 只属于 `APP-SET-08` 增量六态前历史基线。原型用于确认视觉与交互；未读、权限、目标状态、余额、分录与冲正关系仍必须以服务端实时结果为准。
...
- 50 个移动端 Page ID、49 个管理后台 Page ID、P0/P1/P2 的 56/32/7 页面数量与需求追踪矩阵一致。
- 99 个 Page ID 均包含产品需求编号、发布范围编号、Feature PRD、默认状态原型和页面级验收；57 个 P0 页面另有关键状态原型，总计 156 张基础原型。
- Figma 最终设计完整覆盖 99 页、408 个需求状态和 99 个流程预览；当前 3,585 个页面内与流程动作已重算且缺失目标为 0，3,571 只保留为增量前历史基线。
- 客户 DOCX 下一次生成将保留 156 张基础逐页图与通知/金币 23 张逐状态导出图，共 179 个确定性图片映射；同名状态优先使用逐状态导出图。
...
| C-07 | 研发优先级 | P0/P1/P2 分层；99 页完整设计覆盖不等于首批同时开发 |  |
...
以上图片来自 [App 1.0 高保真关键旅程原型](./interactive-prototype/index.html)，用于快速确认主流程。完整的 [99 页逐页交互设计库](./interactive-prototype/pages.html) 用于逐页确认信息结构、主次操作和必备状态；Figma 最终文件负责像素级视觉、完整状态和原型连线。[逐页产品与交互设计](./APP_PAGE_LEVEL_PRODUCT_DESIGN.md) 提供页面目录、优先级和验收方法；[详细功能与逐页原型说明](./APP_DETAILED_FUNCTION_PROTOTYPE_SPEC.md) 逐页嵌入 156 张基础原型，并对通知与金币 5 页使用 23 张逐状态导出图；[需求追踪矩阵](./APP_REQUIREMENTS_TRACEABILITY.md) 记录产品需求、发布范围、Feature PRD 与 Page ID 的对应关系。客户 DOCX 附录 A/B/C 下一次生成会按 Page ID 展示 179 个图片映射和确认栏，Figma 的 408 个最终状态仍是实现视觉与交互的权威来源。
