...
配套的 [App 1.0 高保真关键旅程原型](./interactive-prototype/index.html) 可直接操作登录、发现、详情、会员门槛、平台会话、通知钱包、内容审核和运营调币八个场景。[App 1.0 逐页交互设计库](./interactive-prototype/pages.html) 进一步覆盖移动端 49 页和管理后台 43 页，共 92 个可独立访问的页面设计，并支持切换每页的正常、空、错误、受限和冲突状态。详细页面基线见 [逐页产品与交互设计](./APP_PAGE_LEVEL_PRODUCT_DESIGN.md)。

### 1.1 本次要求客户重点确认

1. 普通用户注册后只是观看者，不会创建或绑定真人资料，也不会作为真人出现在列表中；仍可浏览已发布真人资料。
2. 只有管理员认证并发布的真人资料才能公开展示。
3. 当前多数真人尚未由本人运营，私信由平台运营人员接收和回复。
4. 平台运营身份必须持续披露，不得表现为真人本人在线、输入、已读或回复。
5. 只有有效心享会员可以创建和发送私信，不需要双方同意或匹配。
6. 五级会员全部在 1.0 展示并支持管理员手动发放，1.0 不提供在线购买。
...
平台首期的供给侧仍由管理员管理。普通注册用户只浏览、互动和使用会员私信；未来真人本人可在完成认领、身份核验和授权复核后接收新会话，但该能力不属于 App 1.0。

### 2.2 产品定位

面向观看者的真人发现与互动平台：用户可以按地区、热度、职业、风格和偏好发现经平台审核发布的真人资料，进行喜欢、关注、收藏，并在持有有效会员时向目标真人发起由平台运营接收的私信。

### 2.3 业务目标

- 建立“真人资料供给 → 审核发布 → 用户发现 → 单向互动 → 会员私信 → 平台运营”的完整闭环。
...
- 用户可以独立完成发现、关注/收藏、会员门槛理解、阅读/发送私信、举报和账号管理。
...
- 喜欢、关注、收藏、收藏夹、浏览历史。
- 五级心享会员目录、当前权益、管理员手动发放和自动到期。
- 会员创建/发送文本私信、会话列表、站内实时刷新和平台代运营工作台。
...
未来可以通过新 App 版本增加在线购买、充值、礼物、装扮、媒体消息、系统推送、真人认领和用户桌面端。服务端可提前保留稳定 capability/entitlement key，但 1.0 客户端不得出现不可执行入口、占位按钮或促销文案。
...
| 角色 | 主要任务 | 关键限制 |
|------|----------|----------|
| 访客 | 查看品牌、协议和有限公开预览 | 不能互动、查看私有数据或私信 |
| 普通观看者 | 浏览真人列表，进行发现、搜索、喜欢、关注、收藏、举报和设置 | 不能私信，也不能创建、绑定或进入真人资料管理列表 |
| 心享会员 | 普通观看者能力 + 私信和已配置会员权益 | 不保证回复，不改变认证结果 |
...
| 平台 | 页面数 | 设计范围 |
|------|--------|----------|
| Android/iOS 移动端 | 49 | 启动认证、发现真人、单向互动、平台私信、站内通知、五级会员、金币钱包、我的与系统状态 |
| Nuxt 管理后台 | 43 | 总览异常、真人供给、认证发布、目录推荐、代运营、安全申诉、会员金币、通知审计 |

每个页面必须有稳定 Page ID、设计路由、页面目标、进入方式、主次操作、必备状态、成功出口、失败下一步和页面级验收。客户可在 [逐页设计库](./interactive-prototype/pages.html) 搜索 Page ID 并单独评审；评审意见应引用 Page ID，避免只描述截图位置。
...
6. 授权撤回、认证过期、争议或安全事件触发暂停，停止新曝光、新私信和新媒体凭证。
...
4. 普通观看者点击私信时进入会员说明，不创建会话、不消耗额度。
5. 有效会员确认接收主体和额度后创建或复用会话。
6. 未认领真人的消息进入平台运营队列，由管理员以平台运营身份回复。
7. 会员到期后会话历史仍可读，输入区变为只读。

### 6.3 会员与金币管理流程

1. 管理员选择会员等级、有效期、来源和用户可见说明，提交发放。
2. 高风险或批量操作按策略进入独立复核；执行成功后更新 entitlement 并通知用户。
3. 管理员创建加币、扣币、补偿或冲正申请，预览预计余额。
4. 达到金额、频率或批量阈值时由不同复核人批准。
5. 执行只追加 WalletEntry；原分录不可修改或删除。
6. 用户只看到已生效余额、分录原因、时间和必要的业务引用。

## 7. 五级心享会员方案

### 7.1 等级命名与定位

| rank | 名称 | 客户展示定位 | App 1.0 私信 |
|------|------|--------------|----------------|
...
| 权益 | 心遇 | 心悦 | 心知 | 心契 | 心耀 |
|------|------|------|------|------|------|
| 每日新会话次数 | 2 | 5 | 10 | 20 | 40 |
| 已有会话发送文本 | 允许 | 允许 | 允许 | 允许 | 允许 |
| 保存筛选条件上限 | 1 | 3 | 6 | 12 | 20 |
| 收藏夹数量上限 | 3 | 5 | 10 | 20 | 30 |
| 高级筛选 | 基础组合 | 开放 | 开放 | 开放 | 开放 |
| 未来权益挂载 | 支持 | 支持 | 支持 | 支持 | 支持 |

说明：

- “新会话次数”只在首次成功创建会话时消耗；已有会话复用、重复点击或网络重试不重复消耗。
- App 1.0 不设置对用户展示的“无限消息”。已有会话发送受统一安全频控、内容审核、会话状态和会员有效期控制。
- 额度重置建议按用户服务地区每天 00:00 执行，跨时区与地区规则在上线前确认。
- 上表是可落地的推荐默认值，客户可在确认页修改；实现后由服务端配置，不在客户端硬编码名称和数值。

### 7.3 会员页面规则

- 同时展示五级等级、当前等级、有效期、已用/剩余额度、获取方式和限制。
- 免费用户的主按钮为“了解获取方式”，不是“立即购买”。
- App 1.0 不显示价格、折扣、续订、恢复购买、退款或倒计时促销。
- 所有等级必须展示“消息由平台运营接收”“不保证回复时间或关系结果”。
...
- 搜索历史支持逐条和全部清除，自由文本不得进入普通分析日志。

### 8.4 真人详情与媒体

![真人详情、私信门槛与五级会员原型](./assets/client-prd/prototype-02-profile-membership.png)

详情顺序：媒体头图 → 认证与展示名 → 模糊地区/职业/标签 → 简介 → 图库 → 消息接收主体说明 → 喜欢/关注/收藏 → 私信 → 举报/拉黑/分享。

关键交互：

- “资料已认证”可点击查看平台实际核验范围和更新时间。
- “平台运营接收”使用不同颜色、图标和说明，不得与认证徽章合并。
- 媒体只展示审核后的派生资源；受保护媒体先请求短期凭证。
- 凭证过期且权限仍有效时可静默刷新一次；资料暂停、授权撤回或会员失效时停止刷新并清理保护缓存。
- 分享链接每次打开重新校验公开状态；下架后只显示通用不可用说明。
- 拉黑前说明将停止推荐、互动和私信；解除拉黑不自动恢复互动或会话。
...
用户从真人详情点击私信后，服务端返回目标状态、消息接收主体、会员资格、额度和限制。
...
会话列表展示真人展示名、平台运营标签、最近消息安全摘要、时间、未读数、静音和限制状态。
...
推荐默认交互（待客户确认）：消息发送后 2 分钟内允许撤回，撤回后保留“消息已撤回”占位；关闭会话后变为只读，重新开启需要用户主动操作并重新校验会员、资料、拉黑和安全状态，同一会话不重复消耗新会话额度。
...
发布前必须满足：认证有效、用途授权有效、公开字段完整、媒体安全、目录有效、运营归属明确、私信接收主体说明正确、举报入口可用。

发布通过后生成公开投影。投影失败时保持上一完整版本或不可见，不允许一半新一半旧。

紧急暂停优先停止推荐、搜索、详情、新私信和新媒体凭证。暂停不删除资料、历史消息和审计。恢复前必须重新通过当前认证、授权和发布门禁。

### 9.4 标签、地区、分类与推荐运营

- 目录项使用稳定 ID、类型、父级、别名、状态和版本。
- 合并或下线目录项时预览受影响资料、筛选和保存条件。
- 热门使用版本化聚合规则和时间衰减，不直接以消费额、会员等级或私信数排名。
...
- 推荐服务时间：每日 10:00–22:00；对用户说明服务时段但不承诺固定回复时限。具体排班待客户确认。
...
发放表单包含：等级、生效时间、到期时间/期限、来源、业务单号、原因、用户可见说明和内部备注。提交前预览旧权益、新权益和对私信/额度的影响。
...
- 认证：“资料已认证”“查看认证范围”。
- 平台运营：“消息由平台运营接收和回复”“平台运营已读”。
- 会员门槛：“需要有效心享会员”“了解获取方式”。
...
- 供给：认证发布真人数、授权完整率、审核时长、暂停和争议率。
- 发现：合规曝光、详情访问、搜索成功、无结果、关注和收藏转化。
- 私信：会员门槛曝光、会话创建、额度用量、平台首次响应、有效往返和关闭率。
- 会员：各级发放、生效、到期、撤销和私信使用。
...
- 五级会员同时可见并支持管理员手动发放，App 不显示购买入口。
- 无有效会员时点击任一私信入口不会创建会话或消耗额度。
...
- 紧急暂停后停止公开曝光、新私信和新媒体凭证，但不破坏历史证据。
...
以下事项需要客户在进入高保真和实现前确认。未确认时采用“建议方案”进行设计，不进入不可逆实现。

| 编号 | 确认事项 | 建议方案 | 客户确认/调整 |
|------|----------|----------|---------------|
| C-01 | 正式产品名、Logo、域名和应用 ID | “心动遇见你”为工作名，完成商标/商店检索后确定 |  |
| C-02 | 首发地区和语言 | 单一地区、简体中文；地区需完成数据与合规评估 |  |
| C-03 | 年龄门槛 | 18 周岁及以上 |  |
| C-04 | 登录方式 | 根据首发地区启用手机号/邮箱；第三方登录单独确认 |  |
| C-05 | 真人认证公开文案 | “资料已认证”，详情展示实际核验范围，不宣称本人已入驻 |  |
| C-06 | 上传、认证、发布职责分离 | 内容编辑、认证审核、发布审核分离；高风险双人复核 |  |
| C-07 | 五级会员额度 | 采用第 7.2 节推荐默认值 |  |
| C-08 | 额度重置时区 | 用户服务地区每日 00:00 |  |
| C-09 | 平台运营服务时间 | 每日 10:00–22:00，明确不承诺固定回复 |  |
| C-10 | 消息撤回窗口 | 发送后 2 分钟，保留撤回占位 |  |
| C-11 | 关闭会话后重新开启 | 用户主动开启并重新校验，不重复消耗同一会话额度 |  |
| C-12 | 文本审核 | 规则拦截 + 人工队列；高危内容先审核后投递 |  |
| C-13 | 调币负余额 | 默认禁止执行后余额小于 0 |  |
| C-14 | 调币复核阈值 | 单笔 >1,000、24 小时累计 >2,000 或批量 >20 触发复核 |  |
| C-15 | 消息、证据和审计保留期 | 按首发地区法务/隐私结论分别确定 |  |
| C-16 | iOS 最低版本与目标商店 | 工程 Spike 与目标商店要求确认 |  |
| C-17 | Cloudflare 数据位置与预算 | 生产接入前完成书面结论和容量压测 |  |
...
|------|----------|
| 原型 01 | 登录、推荐、搜索与筛选 |
| 原型 02 | 真人详情、接收主体、私信确认、五级会员 |
| 原型 03 | 会话列表、平台运营聊天、会员到期只读 |
...
以上图片来自 [App 1.0 高保真关键旅程原型](./interactive-prototype/index.html)，用于快速确认主流程。完整的 [92 页逐页交互设计库](./interactive-prototype/pages.html) 用于逐页确认信息结构、视觉方向、主次操作和必备状态；[逐页产品与交互设计](./APP_PAGE_LEVEL_PRODUCT_DESIGN.md) 提供对应的书面页面目录和验收方法。后续研发可在不改变已确认身份披露、权限门槛、状态和业务规则的前提下，继续细化字体、颜色、图片、间距和动效参数。
