...
QA 证据分两段：原 402/402 正式状态与 3,571 个有效交互源已完成全量检查，缺失目标、移动端关键点击热区、文字样式、字体、字符图标和文字溢出问题均为 0；`APP-SET-08` 新增六态已完成 393 × 852 定向文字、边界与原型检查。当前 408 状态动作总数与全量 QA 统计在全部开发完成后统一重算。客户审阅时可先查看下列最终视觉总览，再到附录 A/B/C 按 Page ID 逐页确认功能和状态。
...
这 23 张逐状态导出图是 408 个 Figma 最终状态中的深度确认样本；当前最终稿覆盖移动端 50 页/208 状态、后台 49 页/200 状态。3,571 个有效交互源及其全量 QA 结论属于 `APP-SET-08` 增量六态前历史基线，新增六态已单独完成定向视觉和原型检查。原型用于确认视觉与交互；未读、权限、目标状态、余额、分录与冲正关系仍必须以服务端实时结果为准。
...
- Figma 最终设计完整覆盖 99 页、408 个需求状态和 99 个流程预览；3,571 个页面内与流程交互源是增量前历史基线，当前总数待开发完成后重算。
