...
- Figma 最终设计覆盖全部 99 个 Page ID 和 408 个正式需求状态，并提供 99 个流程预览；3,571 个页面内与流程动作是 `APP-SET-08` 增量六态前、缺失交互目标为 0 的历史基线，当前动作总数在全部开发完成后统一重算。客户文档下一次生成将保留 179 个 Page ID/状态/图片确定性映射用于离线签署。
...
- **PRD-AC-015**：Figma 最终文件包含 50 个移动端页面/208 个状态、49 个后台页面/200 个状态和 99 个流程预览；原 402 状态与 3,571 个有效交互动作已完成全量 QA，`APP-SET-08` 新增六态已完成 393 × 852 定向文字、边界与原型检查，当前 408 状态动作总数和全量 QA 统计在开发完成后统一重算。
