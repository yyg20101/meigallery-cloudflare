...
“心动遇见你”（工作名）是一款独立的经授权真人内容发现与平台话题 App。平台展示由管理员认证并发布的真人资料，帮助观看者按地区、热度、标签和个人偏好发现感兴趣的真人，进行喜欢、关注、收藏，并由有效心享会员围绕真人资料发起由平台运营接收的话题。礼物和个性装扮属于后续商业化方向，不是 App 1.0 上线范围。

平台当前由管理员维护真人资料，并接收未认领真人相关的平台话题。未来真人本人可通过认领流程运营本人资料并接收新会话。产品不建立普通用户间的公开资料、双方匹配或用户间聊天。
...
| 心享会员 | 持有有效会员 entitlement | 普通能力 + 平台话题、等级额度和高级筛选 |
| 内容管理员 | 管理真人资料和媒体 | 上传、导入、认证、发布、暂停、归档 |
| 平台话题运营 | 处理未认领真人相关话题 | 会话分配、平台身份回复、升级审核和审计 |
...
### 3.3 M2A：App 1.0 平台话题与手动运营

- 心享会员五级全部展示，用户可提交站内申请，由管理员审核发放并设置有效期。
- 有效会员创建平台话题、管理员运营工作台和实时文本消息。
...
- App 1.0 当前定义移动端 50 页、Nuxt 管理后台 49 页，共 99 个稳定 Page ID。
- 99 个页面全部具有默认状态原型；57 个 P0 页面另有一张关键异常、受限、冲突或处理中状态原型，共 156 张逐页原型图。
- Figma 最终设计覆盖全部 99 个 Page ID 和 408 个正式需求状态，并提供 99 个流程预览；2026-08-24 已重算 3,585 个正式页面/流程动作，缺失目标为 0。3,571 只保留为 `APP-SET-08` 增量六态前历史基线。客户文档保留 179 个 Page ID/状态/图片确定性映射用于离线签署。
- 页面优先级为 P0 57 页、P1 32 页、P2 10 页。P0 阻塞限量 Alpha/Beta，P1 阻塞 App 1.0 客户最终验收，P2 为可独立排期的后台运营增强。
...
Search-1 当前已完成 production 默认关闭的公开字段人物搜索和账号私有搜索历史服务端基线：搜索词通过 POST 正文传输，不进入 URL、游标、审计或分析事件。Taxonomy-1 与 Search-2 已继续完成稳定分类目录、组内 OR/组间 AND、父级包含后代的结构化筛选、高级 entitlement 校验、结果数安全预估和账号私有保存条件；受限或失效条件不会被忽略后扩大结果，保存条件不持久化自由搜索词。Search-2 管理端同时提供只读策略、跨域依赖和聚合数据健康核查，不读取搜索词、条件名称或用户明细，也不能直接改变运行配置。Recommendation-1 已完成版本化非个性化/个性化规则、本人主动 taxonomy 偏好、固定“平台精选”披露、合成 Dry-run、稳定灰度、计划生效、暂停和回滚的服务端及 Nuxt 后台开发基线；会员、金币、消息、搜索、浏览、关注、收藏和精确位置均不进入本切片排序。四个切片的 KMP 均已接入，Taxonomy-1、Search-2 与 Recommendation-1 的 Nuxt 管理工作区也已交付；环境配置、migration 与专项验证仍后置。隐式行为信号、正式热度公式和曝光证据须在对应决策关闭后另行启用。详见 [Search-1 人物搜索与搜索历史开发基线](./SEARCH_1_PERSON_SEARCH_HISTORY_INTEGRATION.md)、[Taxonomy-1 稳定分类目录与人物关联开发基线](./TAXONOMY_1_CATALOG_AND_PROFILE_INTEGRATION.md)、[Search-2 结构化筛选、结果预估与保存条件开发基线](./SEARCH_2_FILTERS_AND_SAVED_FILTERS_INTEGRATION.md) 和 [Recommendation-1 版本化推荐与运营精选开发基线](./RECOMMENDATION_1_RULES_AND_EDITORIAL_INTEGRATION.md)。
...
总纲见 [真人发现、单向互动与平台话题 PRD](../ways-of-work/plan/real-person-discovery-platform/managed-person-discovery-and-messaging/prd.md)；App 1.0 可验收细节见 [会员平台话题、实时会话与运营工作台 PRD](../ways-of-work/plan/real-person-discovery-platform/member-messaging-and-managed-operations/prd.md)。
...
Membership-6 已交付默认关闭的管理员批量发放服务端契约：固定 CSV 生成不可变逐行预览，有效行逐项创建 Membership-3 普通复核申请，部分失败可恢复且不重复成功项。OQ-018、`0104`、配置和验证后置；Figma 没有正式批量页面时不增加 Nuxt UI 或页面状态。实现边界见 [Membership-6 会员批量发放服务端交付基线](./MEMBERSHIP_6_BATCH_GRANTS_INTEGRATION.md)。
...
每周有效发现用户数：一周内完成以下任一条件的去重观看者数：

- 查看至少 2 个不同的已认证真人详情，并完成至少 1 次关注或收藏。
- 创建 1 个合规平台话题，收到至少 1 次有效平台回复，并产生 1 次用户后续回应。

测试账号、管理员账号、被判定为刷量的行为和仅打开页面未产生主动行为的访问不计入。

### 8.2 核心指标

- 供给：已认证发布真人数、授权完整率、审核时长、暂停/争议率。
- 发现：推荐覆盖率、详情访问率、搜索成功率、关注/收藏率、非个性化使用率。
- 平台话题：会员申请转化、会话创建率、首次响应时长、有效往返率、关闭率。
- 1.0 会员与金币：各级申请、手动发放、到期、话题使用、管理员调币和账本差异。
...
- 未披露平台接收、授权缺失上线、管理员冒充本人、账本不一致和越权访问必须为零容忍事件。
