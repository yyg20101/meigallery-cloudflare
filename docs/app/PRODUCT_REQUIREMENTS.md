...
“心动遇见你”（工作名）是一款独立的真人发现与互动 App。平台展示由管理员认证并发布的真人资料，帮助观看者按地区、热度、标签和个人偏好发现感兴趣的真人，并进行喜欢、关注、收藏、会员私信、礼物和个性装扮消费。
...
- 建立五级心享会员与不可提现的虚拟商品体系。
- 以 KMP 支持 Android/iOS 首发和 Windows/macOS 长期演进。
...
- M1/M2 不做直播、公开评论、音视频通话和用户上传公开媒体。
...
| 心享会员 | 持有有效会员 entitlement | 普通能力 + 私信、等级额度、高级筛选和会员商品 |
| 内容管理员 | 管理真人资料和媒体 | 上传、导入、认证、发布、暂停、归档 |
| 代运营管理员 | 处理未认领真人私信 | 会话分配、平台身份回复、升级审核和审计 |
| 商业运营/财务 | 管理权益和账务 | 商品、订单、退款、调币、对账和复核 |
...
- 金币充值、明细、礼物、头像框、主页皮肤和聊天皮肤。
- 管理员加币、扣币、补偿、冲正、复核、订单和退款。
...
- Windows/macOS 客户端、多语言、多地区和平台级实验能力。
...
桌面端采用左侧主导航与“列表 + 详情”布局，保持相同能力和服务端权限。
...
- **PRD-FR-054**：消息支持发送、失败、审核中、送达、已读、撤回规则、举报、拉黑、静音和关闭状态。
...
- **PRD-FR-060**：首版同时展示和销售心遇、心悦、心知、心契、心耀五级。
- **PRD-FR-061**：授权使用 rank 与 entitlement，名称、颜色、文案和价格不参与权限判断。
- **PRD-FR-062**：会员页展示价格、期限、续订方式、具体额度、接收主体、限制和条款。
...
- **PRD-FR-071**：礼物为固定价格的非提现互动商品，不向真人产生可提现收入。
- **PRD-FR-072**：头像框、主页皮肤和聊天皮肤支持预览、购买、装备、卸下、期限和下架处理。
- **PRD-FR-073**：订单、扣币、赠礼和发货必须幂等；余额不足或失败时不产生部分扣款。
...
- **PRD-FR-080**：通知按消息、互动、交易/权益、系统/安全和营销分类控制。
- **PRD-FR-081**：交易、安全和账号通知不得因关闭营销通知而丢失。
- **PRD-FR-082**：用户可访问帮助、举报进度、申诉、隐私设置、数据导出和注销。

### 5.10 后台

- **PRD-FR-090**：后台包括真人导入、认证发布、标签地区、推荐位、代运营消息、举报审核、会员商品、调币退款、认领和审计模块。
...
- **PRD-NFR-007**：移动与桌面支持屏幕阅读器、动态字体/缩放、键盘导航、高对比度和减少动态效果。
...
- 商业：各级购买/续订/退款、金币充值和消费、礼物/装扮使用、对账差异。
...
- **PRD-AC-001**：普通账号注册和购买会员后仍不会成为公开真人资料。
...
- **PRD-AC-007**：五级会员都可在首版目录中展示和销售，服务端按 rank/entitlement 授权。
- **PRD-AC-008**：真人认领后新会话路由本人，历史代运营消息不自动开放。
- **PRD-AC-009**：旧客户端安全忽略未知 entitlement，不崩溃也不扩大权限。
- **PRD-AC-010**：任一公开发布候选版本通过安全、隐私、商店、财务、无障碍和恢复门禁。

## 10. 文档关系

本文件是产品层总需求。模块可验收细节以 [Feature PRD 目录](../ways-of-work/plan/real-person-discovery-platform/README.md) 为准；技术边界以 [技术架构](./TECHNICAL_ARCHITECTURE.md)、[数据与迁移](./DATA_AND_MIGRATION.md) 和 [API 契约](./API_AND_REALTIME_CONTRACT.md) 为准。发生冲突时，先更新产品蓝图和决策登记，再同步所有下游文档，禁止在实现中自行选择旧规则。
