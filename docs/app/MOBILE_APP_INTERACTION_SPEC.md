...
50 个移动端 Screen ID 均已进入 [App 1.0 逐页交互设计库](./interactive-prototype/pages.html)，可独立访问、切换必备状态并查看页面目标、入口、主次操作和验收说明；完整目录见 [逐页产品与交互设计](./APP_PAGE_LEVEL_PRODUCT_DESIGN.md)，逐页角色、规则、权限、需求编号、验收与原型见 [详细功能与逐页原型说明](./APP_DETAILED_FUNCTION_PROTOTYPE_SPEC.md)，上游需求对应关系见 [需求追踪矩阵](./APP_REQUIREMENTS_TRACEABILITY.md)。
...
- 喜欢、关注、收藏、浏览历史和拉黑联动。
- 五级心享会员目录、会员申请与进度、当前权益和额度。
- 有效会员创建/发送平台话题、平台代运营披露、文本/表情消息。
- 站内通知中心、金币余额与追加式明细。
...
| `APP-AUTH-02` | `/auth/login` | 登录 | App 1.0 邮箱密码、四类文档入口、帮助 | 输入错误、验证中、频控、账号受限、协议更新 |
| `APP-AUTH-03` | `/auth/register` | 注册 | 邮箱、密码、邮箱验证码、四类文档确认 | 标识占用、验证码失效、地区不可用 |
| `APP-AUTH-04` | `/auth/challenge` | 风险验证 | Cloudflare Turnstile 受控 WebView、人机验证边界、失败重载、取消返回 | 等待、失败、次数限制 |
| `APP-AUTH-05` | `/onboarding/preferences` | 初始偏好 | 地区范围、标签、风格、内容类型、跳过 | 空目录、保存失败、非个性化说明 |
| `APP-AUTH-06` | `/legal/{document}` | 四类当前文档 | 服务条款、隐私政策、平台运营说明、必要资格说明；服务端版本与完整正文；真实返回栈 | 加载中支持稿、加载失败、版本更新 |
...
| Screen ID | 设计路由 | 页面 | 主要内容与操作 | 必备状态 |
|-----------|----------|------|----------------|----------|
| `APP-DSC-01` | `/discover` | 推荐首页 | 44dp 地区/搜索/筛选、推荐/地区/热门/最新、横向重点真人、双列卡片 | 正常、首次空、骨架、分页、离线缓存、规则刷新 |
| `APP-DSC-02` | `/discover/regions` | 地区选择 | 460dp 弹层、隐私说明、当前地区、范围/城市草稿、应用动作 | 正常、定位未使用、目录更新、无结果 |
...
| `APP-DSC-06` | `/saved-filters` | 已保存条件 | 条件列表、额度、使用前复核、名称/条件/默认排序完整编辑、删除二次确认 | 加载/失败、空、额度满、会员降级、引用标签合并/失效、更新/删除失败、版本冲突 |
...
Recommendation-2/3/4 不改变上述 Figma 页面。KMP 已由公共请求构造器发送数字 `X-Client-Version`，并在推荐请求中发送现有规范化 `regionCode`；bootstrap 只在当前版本存在至少一个地区/版本匹配且 taxonomy/heat 依赖完整的非个性化规则时开放推荐 capability。高版本、非目标地区、失效依赖或个性化目录不兼容的规则不会覆盖服务端确认的旧安全规则；未选择地区时只执行全局规则，不由客户端自行选择规则、猜测地区、迁移 taxonomy、比较展示名或放宽资格。

### 5.3 单向互动与历史

| Screen ID | 设计路由 | 页面 | 主要内容与操作 | 必备状态 |
|-----------|----------|------|----------------|----------|
| `APP-INT-01` | `/following` | 关注动态 | 关注更新流、全部/有更新/最近关注筛选、进入详情、确认取消关注 | 首次空、无更新、资料下架、首载/失败、登录失效、能力关闭、分页、取消结果、游标整表刷新 |
| `APP-INT-02` | `/likes` | 喜欢 | 独立账号私有列表、搜索、地区/风格筛选、进入详情、服务端确认后撤销 | 正常、空、资料不可用；另有 16 张加载、受限、筛选、分页、撤销与游标刷新支持稿 |
...
| `APP-INT-06` | `/person/{profileId}/favorites` | 调整收藏夹 | 即时保存人物收藏归属、完成返回 | 加载中、读取失败、更新中、更新失败、成功反馈、资料不可用、移出最后收藏夹确认、移出最后一项中、已取消收藏 |
...
| Screen ID | 设计路由 | 页面 | 主要内容与操作 | 必备状态 |
|-----------|----------|------|----------------|----------|
| `APP-MSG-01` | `/messages` | 平台话题列表 | 会话、平台运营标签、摘要、时间、未读、静音 | 首次空、离线、会话受限 |
| `APP-MSG-02` | `/messages/start/{profileId}` | 发起话题确认 | 接收主体、权益/额度、不保证回复、确认创建 | 无会员、额度尽、资料失效、重复会话 |
| `APP-MSG-03` | `/messages/{conversationId}` | 会话 | 服务说明卡、消息、状态、文本/表情输入、菜单 | 补拉、审核中、只读、冻结、关闭、消息操作、举报消息 |
| `APP-MSG-04` | `/messages/{conversationId}/details` | 会话设置 | 接收主体说明、静音、举报、拉黑、关闭 | 操作失败、已关闭、举报话题、确认屏蔽、确认关闭 |
...
| Screen ID | 设计路由 | 页面 | 主要内容与操作 | 必备状态 |
|-----------|----------|------|----------------|----------|
| `APP-MBR-01` | `/membership` | 五级会员目录 | 五级切换、当前等级、差异、申请入口、限制 | 免费、已提交、处理中、同步失败 |
| `APP-MBR-02` | `/membership/current` | 当前权益 | 有效期、typed entitlement、已用/剩余额度、重置时间 | 即将到期、到期、撤销、受限 |
| `APP-MBR-03` | `/membership/apply` | 会员申请与进度 | 期望等级、已验证联系方式、申请说明、状态时间线 | 未申请、已提交、处理中、待补充、已通过、已拒绝 |
| `APP-WAL-01` | `/wallet` | 金币钱包 | 余额、最后同步、规则、明细入口 | 空钱包、离线缓存、同步失败 |
| `APP-WAL-02` | `/wallet/entries` | 金币明细 | 全部/增加/扣减、数量、原因、时间 | 首次空、分页、对账维护 |
| `APP-WAL-03` | `/wallet/entries/{entryId}` | 分录详情 | 方向、数量、说明、安全业务单号、冲正、申诉 | 分录不可用、扣减 |

### 5.6 我的、设置与数据权利

| Screen ID | 设计路由 | 页面 | 主要内容与操作 | 必备状态 |
|-----------|----------|------|----------------|----------|
| `APP-SET-01` | `/me` | 我的 | 账号摘要、会员、金币、我的喜欢、设备、隐私、帮助 | 账号受限、摘要同步失败 |
...
| `APP-SET-08` | `/me/appeals` | 申诉 | 举报结论、账号限制和金币分录共用独立复核入口；支持创建、幂等重试、进度、补充必要说明、升级复核与终态结果 | 正常 `159:73873`、已有处理中 `159:73925`、提交失败 `159:73978`、补充说明 `1118:3615`、补充提交失败 `1123:3616`、升级处理中 `1123:3668`、维持原结论 `1130:3617`、申诉成立 `1132:3618`、已关闭 `1132:3670` |
| `APP-SET-09` | `/me/data-export` | 数据导出 | 范围说明、创建任务、进度；仅 Privacy-2 服务端能力开放后允许下载 | 处理中、失败、已过期、重新验证、下载能力未开放（支持态 `1064:149`） |
...
规则：大字体、小屏或辅助功能模式切换单列；卡片主体和喜欢热区分离且均不小于 44dp；不显示在线、精确距离或保证回复。频道切换重置游标；分页失败保留列表；离线缓存只能复用完全相同条件；推荐规则变化进入独立刷新态。

### 6.1.1 地区选择 `APP-DSC-02`
...
规则：底部弹层高 460dp、顶部圆角 28dp；遮罩、关闭、范围、城市和应用动作均为独立 44dp 热区。选择只更新草稿，关闭/遮罩放弃，“应用地区”才提交稳定 code；“全国”提交 null。目录更新、定位未使用、无结果分别使用 Figma 独立状态，无结果提示与弹层间距 24dp。
...
“我的喜欢”使用 `APP-SET-01｜我的｜正常` 内正式入口节点 `852:3613`，点击导航到 `APP-INT-02｜喜欢｜正常` `159:66943`。入口只表达当前账号的私有单向喜欢记录，不暗示匹配、对方通知或真人本人互动。
...
详情点击“发起话题”
→ 服务端返回当前接收主体和资格
→ 无会员：打开 entitlement gate，可提交或查看会员申请，不创建会话
→ 有会员：进入建会话确认，披露平台运营、额度和限制
→ 用户确认 → 幂等创建/复用会话 → 打开会话页
```

- entitlement gate 主操作为“查看心享会员”或“提交会员申请”，不是“立即购买”。
...
会员目录 → 选择等级 → 提交会员申请
→ 已提交 / 处理中 / 待补充
→ 管理员通过并发放 → entitlement 刷新 → 当前权益
```

- 同一账号同一时间只允许一个进行中的申请；重复提交返回原申请。
- 页面展示申请编号、提交时间、已验证邮箱、联系时段偏好和用户可见时间线；OQ-010 关闭前只说明人工处理，不展示固定服务时段或处理承诺。
- App 1.0 不要求用户通过外部支付或不透明私聊渠道获取会员。
...
- 拉黑前明确影响：停止推荐、互动和新话题；确认后立即返回安全页面。解除拉黑不恢复互动或会话。
...
| Entitlement Gate | 平台话题/高级筛选等权限不足 | 显示所需 key 的用户价值和获取方式；只有用户主动提交会员申请后才创建申请记录 |
...
- **MUI-AC-002**：普通账号从任一“发起话题”入口都只能进入会员说明/申请，不产生会话或额度消耗。
...
- **MUI-AC-010**：屏幕阅读器和大字体下可完成注册、发现、平台话题门槛、会员申请、阅读会话、举报和注销。
- **MUI-AC-011**：会员申请的已提交、处理中、待补充、已通过和已拒绝状态都有事实说明、时间与安全下一步。
