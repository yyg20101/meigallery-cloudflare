...
状态：最小技术脚手架已落地；业务模块、导航与数据实现仍为冻结候选

## 1. 文档目的

本文把 [移动端页面与交互规格](./MOBILE_APP_INTERACTION_SPEC.md) 转换为 KMP 可实现的模块、依赖、页面状态、导航、本地数据、网络和平台端口。独立仓库 `meigallery-client` 已创建最小工程；未经 Spike 验证的功能库和业务模块仍不得视为依赖锁。
