...
本文把 [移动端页面与交互规格](./MOBILE_APP_INTERACTION_SPEC.md) 转换为 KMP 可实现的模块、依赖、页面状态、导航、本地数据、网络和平台端口。本文不创建工程，也不锁定未经 Spike 验证的依赖版本。
...
这是逻辑边界，不要求第一天建立二十多个 Gradle project。建议脚手架期先按 `core`、`domain`、`feature`、`app` 四组目录建立少量模块；当独立编译、所有权或复用收益明确时再拆物理模块，避免过早增加构建成本。
