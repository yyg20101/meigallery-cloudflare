...
本文、[移动端页面与交互规格](./MOBILE_APP_INTERACTION_SPEC.md)、[管理后台页面与交互规格](./ADMIN_CONSOLE_INTERACTION_SPEC.md) 与逐页设计库共同构成实现前的页面基线。出现冲突时，客户确认 PRD 决定产品边界，页面规格决定详细交互，服务端契约决定最终业务事实。
...
9. **验收**：入口、状态、权限、文案、反馈、返回和 App 1.0 范围均可验证。
...
3. 对每页确认页面目标、主操作、必备状态、身份披露和下一页面。
4. 意见必须引用 Page ID，例如“APP-MSG-02 的无会员状态需调整”，避免只使用截图位置描述。
