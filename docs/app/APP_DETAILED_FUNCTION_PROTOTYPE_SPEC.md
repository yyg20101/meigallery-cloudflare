...
Figma 最终设计已覆盖移动端 50 页、管理后台 49 页和全部 408 个正式需求状态，并建立 99 个流程预览。2026-08-24 实时复核得到 3,585 个正式页面/流程动作（页面 2,971、流程 614），缺失目标为 0；3,571 仅保留为 APP-SET-08 增量六态前历史基线。客户文档保留 99 张默认状态、57 张 P0 关键状态和通知/金币 23 张逐状态注册导出，共 179 个 manifest 确定性图片映射；APP-DSC-01 至 APP-DSC-09、APP-INT-06 与 ADM-PRI-01/02 的逐状态 Figma 图直接进入本 MD。图片均通过 Page ID、状态与 Frame ID 关联，不通过章节位置猜测。
