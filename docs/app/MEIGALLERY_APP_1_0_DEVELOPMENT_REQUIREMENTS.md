...
- Privacy-1 已完成数据权利控制面；Media-1 将累计 App API v2 提升为 `1.18.0`，完成默认关闭的人物图片列表、会员短期取图、公开认证说明、KMP 自适应媒体浏览和媒体举报。它复用现有图库与公开投影，不新增 schema；视频、环境配置、专项测试、Gradle、模拟器/真机和联调统一后置。
