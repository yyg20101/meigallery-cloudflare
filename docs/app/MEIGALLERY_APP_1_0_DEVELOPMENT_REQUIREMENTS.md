...
- Taxonomy-1 已完成稳定词条 ID、不可变修订与目录快照、合并重定向、legacy 显式待复核映射、人物内容版本关联、发布门禁和公开分类投影的服务端代码，以及 Recommendation/Search 共用的 KMP 通用目录、缓存和 ETag 条件重验证。Nuxt 已交付 `ADM-TAX-01/02/03` 三页工作区，包含词条树、草稿/审核/生命周期/合并、映射治理、快照检查和不可变发布。真实目录、环境配置、`0081` 执行、专项测试、模拟器/真机与远端联调继续后置，当前 `taxonomy.catalog=false`。
- Search-2 已完成组内 OR/组间 AND、父级包含后代的 taxonomy 结构化筛选、高级 entitlement 服务端校验、结果数安全预估、目录重定向/失效诊断、账号私有保存条件 CRUD 及对应 KMP 页面。保存条件不持久化自由搜索词，创建原子受 `discovery.saved_filter.max` 限制，使用前按完整来源 stable ID 重新 preview；环境配置、`0082` 执行、真实目录/grant 迁移、Nuxt 管理页面、专项测试、模拟器/真机与远端联调后置，当前 `search.filters=false`、`search.savedFilters=false`。
