...
- 已完成的既有跨仓切片具备对应代码和测试；Interaction-2/3、Search-1/2 与 Taxonomy-1 当前仅完成服务端开发代码，Recommendation-1 已完成 Nuxt 与 KMP 开发，但不能据此推断配置、专项测试或联调已完成。production 的 Auth、会员、消息、安全、搜索、taxonomy、推荐、互动收藏/历史/关注更新、后台和 production-ready 开关及新目录选择保持关闭；既有 dev 联调状态不向这些新切片扩散。保留策略仍为 `unresolved`，不导入真实数据、不执行自动清理，也不代表生产发布授权。
