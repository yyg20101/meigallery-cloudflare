...
- M0、M1、Auth-1、Interaction-1 与 Membership-1 已形成默认关闭的开发基线。
- Message-1 已实现 `APP-MSG-01` 列表、`APP-MSG-02` 人物详情内二次披露确认、`APP-MSG-03` 仅文本会话核心状态，以及 `ADM-MSG-01/02` 的单队列运营工作台最小闭环。
- 本切片只包含 HTTP 手动刷新、创建/复用、日额度、文本发送、已读和平台运营回复；不表示 `APP-MSG-04` 会话设置、通知、举报/拉黑、撤回、实时通道、多操作员分配或质量看板已经完成。
- 服务端、Nuxt 与 KMP 均已具备代码和测试，但 production/dev 消息开关、后台开关、production-ready 和新目录选择保持关闭；没有执行远端 migration、部署或真实数据导入。
- Message-1 交付与后续差距以 [Message-1 跨仓集成边界](./MESSAGE_1_CROSS_REPO_INTEGRATION.md) 为准，不能仅凭页面存在判断功能可上线。
