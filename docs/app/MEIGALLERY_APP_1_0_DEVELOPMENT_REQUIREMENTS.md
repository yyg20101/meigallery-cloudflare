...
- Recommendation-1 已完成统一公开资格、版本化规则、非个性化/个性化隔离、本人显式 taxonomy 偏好、可解释原因、会话级稳定灰度、未来排期、暂停/回滚和固定“平台精选”披露的 API/D1 开发基线，累计 App API v2 为 `1.16.0`；Nuxt 已交付规则列表、规则详情、Dry-run 和精选排期四个后台页面。OQ-023 未批准前个性化不可启用，会员、金币、消息、精确位置和隐式行为不进入信号；环境配置、`0083` 执行、KMP 页面、专项测试与联调后置，当前 `recommendation.feed/preferences/personalization/editorial=false`。
...
- 已完成的既有跨仓切片具备对应代码和测试；Interaction-2/3、Search-1/2 与 Taxonomy-1 当前仅完成服务端开发代码，Recommendation-1 已额外完成 Nuxt 运营后台，但都不能据此推断 KMP、配置或验证已完成。production 的 Auth、会员、消息、安全、搜索、taxonomy、推荐、互动收藏/历史/关注更新、后台和 production-ready 开关及新目录选择保持关闭；既有 dev 联调状态不向这些新切片扩散。保留策略仍为 `unresolved`，不导入真实数据、不执行自动清理，也不代表生产发布授权。
