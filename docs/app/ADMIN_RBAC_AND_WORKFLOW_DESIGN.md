...
Audit-2 已把该边界具体化：admin 只能冻结本人范围，Owner 可冻结跨域范围但申请人与复核人必须不同；申请、复核和发票各自重新验证密码。通过复核和发票前都重算权限指纹与精确事件摘要，变化即失效。CSV 每行水印并复用服务端字段级脱敏，文件不产生公开 URL；只有原申请人能用五分钟一次性 header 票据经 Worker 下载。当前实现细节见 [Audit-2 受控审计导出开发基线](./AUDIT_2_CONTROLLED_EXPORT_INTEGRATION.md)，migration、正式保留/物理清理和专项测试仍后置。
