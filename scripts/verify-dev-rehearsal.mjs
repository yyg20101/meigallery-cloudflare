...
import { createStep, runCommand } from './release-verification-lib.mjs'
...
export async function runDevRehearsalVerification(options = {}) {
  ...
  const apiHealthStep = await requestJsonStep(fetchFn, 'dev-api-health', `${apiUrl}/api/health`, {}, (body) => {
  ...
  const webHealthStep = await requestTextStep(fetchFn, 'dev-web-health', webUrl, {}, (html) => {
  ...
  const contactStep = await postConversion(fetchFn, apiUrl, 'dev-conversion-contact', {
    actionType: 'contact',
  ...
  const startTrialStep = await postConversion(fetchFn, apiUrl, 'dev-conversion-start-trial', {
  ...
  const completeRegistrationStep = await postConversion(fetchFn, apiUrl, 'dev-conversion-complete-registration', {
